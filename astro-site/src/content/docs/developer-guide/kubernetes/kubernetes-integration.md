...
VergeOS provides native Kubernetes integration through a CSI storage driver and a Cloud Controller Manager (CCM). These components connect any Kubernetes cluster running on VergeOS VMs — whether provisioned through [Rancher](rancher-integration), kubeadm, or any other method — to the underlying VergeOS platform.
...
<details>
<summary>Why Not Longhorn?</summary>
...
The Kubernetes Cluster Autoscaler works with VergeOS-backed clusters provisioned through [Rancher](rancher-integration). It automatically adjusts the number of nodes in a pool based on pending pod resource requests — scaling up when pods can't be scheduled and scaling down when nodes are underutilized. See [Documentation and Resources](#documentation-and-resources) below for the upstream project link.
...
- New to Rancher on VergeOS? See the [Rancher Integration](rancher-integration) guide for cluster provisioning, template VM preparation, and the node driver setup.

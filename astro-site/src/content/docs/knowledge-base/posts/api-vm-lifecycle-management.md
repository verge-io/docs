...
slug: knowledge-base/vm-lifecycle-management
...
|-------|-------------------|---------------|---------------|
| **Creation** | Create VM, Add drives, Add NICs, Add devices | `POST /api/v4/vms`, `POST /api/v4/machine_*` | [VM Creation](api-vm-creation) |
| **Power** | Start, Stop, Reboot, Monitor | `POST /api/v4/vm_actions`, `GET /api/v4/machine_status` | [VM Power Management](api-vm-power-management) |
| **Config** | Update CPU/RAM, Manage drives, Manage NICs | `PUT /api/v4/vms`, `POST/PUT/DELETE /api/v4/machine_*` | [VM Configuration](api-vm-configuration) |
| **Advanced** | Clone, Snapshot, Delete, Troubleshoot | `POST /api/v4/vm_actions`, `DELETE /api/v4/vms` | [VM Advanced Operations](api-vm-advanced-operations) |
...
1. [Create VMs](api-vm-creation) - REST API calls to create virtual machines
2. [Power Control](api-vm-power-management) - API endpoints to start, stop, and manage VMs
3. [Configure VMs](api-vm-configuration) - API methods to modify VM settings and hardware
4. [Advanced Operations](api-vm-advanced-operations) - API calls for cloning, backup, and troubleshooting

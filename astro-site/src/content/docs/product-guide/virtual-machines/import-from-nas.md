...
:::caution[Import should be performed from VMs that are powered down.]
:::

:::note
NAS Import is only for VMs, not disks. This process is for VMX or OVF files only. Any other formats will need to [Importing VMs from Files](/knowledge-base/import-vms-from-files).
:::

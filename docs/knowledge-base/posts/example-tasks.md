...
!!! info "Key Points"
    * The **VergeOS Task Engine** allows you to automate operations, triggered by specific events or scheduled times. Using modular, reusable components (tasks, events, schedules, and webhooks) you can configure automation tailored to your environment
    * The following example displays the use of tags, tasks, events and schedules used together to efficiently bring workloads online and offline as they are needed, improving resource efficiency 
...
??? step-detail "1. Create a Tag Category and Tag"
    ...
    **A tag category organizes related tags. We then, we create a specific tag within that category to designate the VMs controlled by this automation.** 
    
    **New Category**: System > Tags > New   
    ...
    **New Tag**: Double-click category created above > New   
...
??? step-detail "4. Configure an Event Trigger for User Login"
    ...
    **Here we define the activity that will invoke the task.**
...
??? step-detail "6. Configure an Event Trigger for User Logout"
    ...
    **This defines a system activity that will launch the task.** 

    From the new task dashboard:  
    **Event Triggers** > **New**

    ![Event trigger user logout - Light Mode](/product-guide/screenshots/jthompson-event-poweroff-light.png#only-light)

    ![Event trigger user logout - Dark Mode](/product-guide/screenshots/jthompson-event-poweroff-dark.png#only-dark) 

??? step-detail "7. Create a Schedule for Fridays at 6:00pm"

    **Creating a schedule allows us to define specific dates/times.  After creating the schedule it can be applied to multiple automated tasks.**
...
??? step-detail "8. Create a Schedule Trigger for the Power Off"
    ...
    **We apply the recurring schedule (Fridays at 6pm) to the task of automatically powering off the VMs.**

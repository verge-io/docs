...
* **Allocate adequate RAM for vSAN** — VergeOS auto-reserves a bare minimum, but performance improves significantly with additional allocation. Use the **Storage buffer per node** setting in [Cluster Settings](/product-guide/system/cluster-settings/) to increase this. General guidelines: 0.5 GB/TB for archive/cold storage, 1.5 GB/TB for controllers, 1 GB/TB for standard-use storage nodes, 1–2 GB/TB for high performance. 

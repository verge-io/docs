...
**Secure Inter-Host Communication:** A tenant can be securely scaled across multiple physical hosts, with its tenant nodes communicating via the tenant's protected encapsulated network.

**Matched Resource Allocation**: Tenant nodes can be deployed across clusters or hosts with different hardware configurations (including specialized equipment like vGPUs), to match varied workload requirements within the tenant.
...
!!! tip "[This KB article](/knowledge-base/determine-node-where-vm-runs) provides information regarding HA Groups"

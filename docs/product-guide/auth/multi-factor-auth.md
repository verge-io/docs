...
Two-factor authentication (**also referred to as MFA or Multi-factor authentication**) is an option that provides an added layer of security for user logins. When two-factor authentication is enabled, login requires authorization via a TOTP authenticator program or the user's email address, in addition to username and password.

!!! warning "TOTP Preferred"  
    While using email-based two-factor authentication does provide some extra protection, the Time-Based One-Time Passwords (TOTP) method ensures much stronger security and protects against phishing and other email-based threats.  Additionally, TOTP generates secure one-time passwords directly on a device, ensuring access even when you're offline. This can be particularly important for administrator users that may need to access a system during Internet connectivity or service issues.

## Two-factor Authentication Using a TOTP Application (preferred method)

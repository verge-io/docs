...
3. Establish settings:
    * **Use TLS:** enable/disable to correspond with relaying system settings when using an email relay host
    * **Hostname:** 
    * **From Name:** specifies the display from name that will appear in email messages
    * **From Address:** specifies the source email address to appear in email messages
    * **Relay Host:** specify the address of the email relay system
    * **Relay Port:** TCP port used by relay host, ex. 25 (unencrypted), 587 (TLS), 465 (SSL/legacy-deprecated)
    !!! tip "VergeOS SMTP can be configured to send from the native, built-in SMTP server or through a relay email server.  Outgoing SMTP from your own environment (rather than using a relay) can involve complex and error-prone configuration, e.g. SMTP settings, firewall rules, DKIM and reverse DNS records, etc. Many customers find using a commercial email relay service to be a preferred alternative for ease of use and improved deliverability, security, and compliance reasons."
    * **Use SASL Authorization:** enable/disable to correspond with relaying system settings when using an email relay host
...
SMTP reports provide a summary of SMTP activity, with information such as the number of messages by recipients and senders, warnings, erros, bytes sent/received, etc. 
Use left menu options to run **Today's Report** or **Yesterday's Report**. 

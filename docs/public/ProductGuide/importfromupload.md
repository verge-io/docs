...
1.  Upload configuration and disk image files to the vSAN. For instructions on uploading files to the vSAN, see [**Media Images**](/docs/product-guide/uploadingtovSAN).
...
8.  Select ***Preferred Tier***, or leave at --default--. This determines the tier first attempted for VM storage. See the [**Preferred Tier**](/docs/product-guide/preferredtiers) page for more information.
...
1.  Upload disk image files to the vSAN. For instructions on uploading files to the vSAN, see [**Media Images**](/docs/product-guide/uploadingtovSAN)
2.  [**Create a new Custom VM**](/docs/product-guide/creatingvms#create-a-new-custom-vm), assigning appropriate hardware specifications and NIC device(s).
...
5.  Select drive ***Media File*** (\*.vhd, \*.vhdx, \*.qcow, raw, etc.) from the dropdown list. ([**VM Drives**](/docs/product-guide/VMdrives) provides detailed drive creation instructions.).

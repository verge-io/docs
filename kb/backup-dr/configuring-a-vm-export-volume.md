...
2. Configure the volume:
    ...
    * **Quiesced**: Typically should be selected to provide application-consistent VM snapshots. ⚠️ **[VM Guest Agent](https://docs.verge.io/product-guide/virtual-machines/vm-guest-agent/) must be installed and registered to provide a quiesced VM snapshot.**
...
Instructions for creating an NFS share can be found [here](https://docs.verge.io/product-guide/nas/nas-shares/#create-an-nfs-share).
...
1. Create a [Remote Volume](https://docs.verge.io/product-guide/nas/nas-remote-volumes/) to mount the external volume to the VergeOS NAS (requires standard NFS or CIFS access)
2. Create a [Volume Sync](https://docs.verge.io/product-guide/nas/volume-syncs/) to synchronize data to the Remote Volume.  Volume syncs can be started on-demand manually and can also be scheduled using the Start Profile setting.
...
VergeOS includes multiple pre-installed schedules (e.g., "Daily at midnight"). Refer to the [Schedules Guide](https://docs.verge.io/product-guide/automation/schedules/) for instructions on creating custom schedules.
{% endhint %}
